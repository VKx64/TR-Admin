      ...
      <CardContent>
        {summaryData && (
          <div className="space-y-6">
            ...
            {/* Fleet Performance */}
            <div>
              <h4 className="font-semibold mb-3">Fleet Performance</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-gray-50 p-3 rounded-lg text-center">
                  <div className="text-lg font-bold text-gray-700">
                    {summaryData.fleet.activeTrucks}/{summaryData.fleet.totalTrucks}
                  </div>
                ...
                <div className="bg-blue-50 p-3 rounded-lg text-center">
                  <div className="text-lg font-bold text-blue-600">
                    {summaryData.fleet.avgEfficiency.toFixed(1)} km/L
                  </div>
                  <div className="text-sm text-gray-600">Fleet Average</div>
                </div>

                {summaryData.fleet.mostEfficient && (
                  <div className="bg-green-50 p-3 rounded-lg text-center">
                    <div className="text-lg font-bold text-green-600">
                      {summaryData.fleet.mostEfficient.efficiency.toFixed(1)} km/L
                    </div>
                    <div className="text-sm text-gray-600">
                      Best: {summaryData.fleet.mostEfficient.plate}
                    </div>
                  </div>
                )}

                {summaryData.fleet.leastEfficient && (
                  <div className="bg-red-50 p-3 rounded-lg text-center">
                    <div className="text-lg font-bold text-red-600">
                      {summaryData.fleet.leastEfficient.efficiency.toFixed(1)} km/L
                    </div>
                    <div className="text-sm text-gray-600">
                      Needs Attention: {summaryData.fleet.leastEfficient.plate}
                    </div>
                  </div>
                )}
              </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {summaryData.comparisons.costChange > 10 && (
                  <Badge variant="destructive" className="mr-2">
                    High cost increase this month (+{summaryData.comparisons.costChange.toFixed(1)}%)
                  </Badge>
                )}
                {summaryData.comparisons.costChange < -10 && (
                  <Badge variant="success" className="mr-2">
                    Good cost reduction this month ({summaryData.comparisons.costChange.toFixed(1)}%)
                  </Badge>
                )}
                {summaryData.projections.weeklyTrend > 15 && (
                  <Badge variant="destructive" className="mr-2">
                    Rising weekly expenses (+{summaryData.projections.weeklyTrend.toFixed(1)}%)
                  </Badge>
                )}
                {summaryData.fleet.avgEfficiency < 8 && (
                  <Badge variant="warning" className="mr-2">
                    Fleet efficiency below industry average
                  </Badge>
                )}
                {summaryData.fleet.avgEfficiency > 12 && (
                  <Badge variant="success" className="mr-2">
                    Excellent fleet efficiency performance
                  </Badge>
                )}
              </div>
            </div>
